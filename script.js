...
const NUMWORDS = sentences[0].length + sentences[1].length + sentences[2].length + sentences[3].length + sentences[4].length;
let i = 0;
  ...
  $(document).on({
    ...
    keydown: function (event) {
      ...
      if (!gameOver && i >= sentences.length) {
        ...
        //DISPLAY WPM
        $('#sentence').text('Ran out of sentences! And you had ' + wpm + ' words per minute!');
        //EMPTY TARGET LETTER
        $('#target-letter').text(' ');
